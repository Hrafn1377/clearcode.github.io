...
const FEEDBACK_CONFIG = {
    ...
    recipientEmail: 'hrafn1377@gmail.com',
...
// ===================================================================
// FIXES FOR MISSING BUTTON EVENT LISTENERS (v3 - CORRECT IDs)
// Fixed: Uses actual button IDs from your HTML
// ===================================================================

// Ensure all button event listeners are set up when page loads
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔧 Setting up missing button event listeners (v3)...');
    
    // FIX 1: Reset All Settings Button (CORRECT ID!)
    ...
    if (resetAllSettingsBtn) {
            ...
            if (confirm('⚠️ Reset all settings to defaults?\n\nThis will:\n- Reset all editor settings\n- Reset visual guide colors\n- Reset accessibility settings\n- Reload the page\n\nThis action cannot be undone.')) {
                try {
                    ...
                    if (typeof showNotification === 'function') {
                        showNotification('Settings reset to defaults. Reloading...', 'success');
                    } else {
        ...
        console.log('✅ Reset All Settings button listener added');
    } else {
        console.error('❌ Reset All Settings button NOT FOUND (ID: resetAllSettings)');
    }
    
    // FIX 2: Clear All Data Button (CORRECT ID!)
    const clearAllDataBtn = document.getElementById('clearAllData');
    if (clearAllDataBtn) {
        clearAllDataBtn.addEventListener('click', function() {
            console.log('Clear All Data button clicked');
            if (confirm('⚠️⚠️ CLEAR ALL DATA? ⚠️⚠️\n\nThis will permanently delete:\n- All settings\n- All open files\n- All saved sessions\n- All stored data\n\nTHIS CANNOT BE UNDONE!\n\nAre you absolutely sure?')) {
                ...
                if (confirm('Final confirmation: Delete everything and start fresh?')) {
                    try {
                        ...
                        alert('Error clearing data. Please try manually.');
                    }
                }
            }
        });
        console.log('✅ Clear All Data button listener added');
    } else {
        console.error('❌ Clear All Data button NOT FOUND (ID: clearAllData)');
    }
    
    // FIX 3: Block Guides Checkbox  
    const blockGuidesCheckbox = document.getElementById('blockGuides');
    if (blockGuidesCheckbox) {
        blockGuidesCheckbox.addEventListener('change', function(e) {
            console.log('Block guides checkbox toggled:', e.target.checked);
            ...
            // Re-apply visual guides if they're enabled
            if (guidesEnabled && editor) {
                console.log('Re-applying visual guides with updated block guides setting');
                // Use the correct function name
                if (typeof updateVisualGuides === 'function') {
                    updateVisualGuides();
                } else if (typeof scheduleVisualGuidesUpdate === 'function') {
                    scheduleVisualGuidesUpdate();
                }
            }
        });
        console.log('✅ Block Guides checkbox listener added');
    } else {
        console.warn('⚠️ Block Guides checkbox not found (ID: blockGuides)');
    }
    
    // FIX 4: Space Guides Checkbox
    const spaceGuidesCheckbox = document.getElementById('spaceGuides');
    if (spaceGuidesCheckbox) {
        spaceGuidesCheckbox.addEventListener('change', function(e) {
            console.log('Space guides checkbox toggled:', e.target.checked);
            if (typeof saveSettings === 'function') {
                saveSettings();
            }
            if (guidesEnabled && editor) {
        ...
        console.log('✅ Space Guides checkbox listener added');
    }
    
    // FIX 5: Indent Guides Checkbox
    const indentGuidesCheckbox = document.getElementById('indentGuides');
    if (indentGuidesCheckbox) {
        indentGuidesCheckbox.addEventListener('change', function(e) {
            console.log('Indent guides checkbox toggled:', e.target.checked);
            if (typeof saveSettings === 'function') {
                saveSettings();
            }
            if (guidesEnabled && editor) {
        ...
        console.log('✅ Indent Guides checkbox listener added');
    }
    ...
    if (resetColorsBtn) {
        resetColorsBtn.addEventListener('click', function() {
            ...
            if (confirm('Reset visual guide colors to defaults?')) {
                // Reset to default colors
                if (typeof guideConfig !== 'undefined') {
                    guideConfig.spaceColor = '#6495ed';
                    guideConfig.blockColor = '#ffa500';
                    guideConfig.pythonColonColor = '#32cd32';
                    guideConfig.indentGuideColor = '#32cd32';
                    ...
                    if (typeof updateColorPickerValues === 'function') {
                        updateColorPickerValues();
                    }
                    if (typeof updateGuideStyles === 'function') {
                        updateGuideStyles();
                    }
                    if (typeof saveSettings === 'function') {
                        saveSettings();
        ...
        console.log('✅ Reset Colors button listener added');
    }
    
    console.log('✅ All button fixes applied successfully (v3)!');
});

console.log('✅ ClearCode main.js fully loaded with all fixes applied (v3)!');